...
int test9()
{
	...
	while (1) {
		...
		if (offset < 0) {  // 没找到数据帧（一般都能找到，毕竟ID3V2数据不多）
			...
			/* 补充数据帧 */
			if (bytesLeft < READBUF_SIZE) {// 【疑问?】1024也是能正常播放的，但是NodeMCU32S上不行
			...
			int errs = MP3Decode(hMP3Decoder, (unsigned char**)&readPtr, &bytesLeft, output, 0);//调用一次解码一帧数据帧
			frames++;
			...
			if (err != ERR_MP3_NONE) {   // 解码错误
				...
				if ((mp3player.samprate != mp3FrameInfo.samprate) || (mp3player.bitsPerSample != mp3FrameInfo.bitsPerSample) || (mp3player.nChans != mp3FrameInfo.nChans)) {
					...
					printf("change mp3player samprate:%d bitsPerSample:%d nChans:%d\n", mp3player.samprate, mp3player.bitsPerSample, mp3player.nChans);
					// Set_WIN_Params(NULL, mp3player.samprate, mp3player.bitsPerSample, mp3player.nChans);
				}
