...
const reflect_item_t stringsReflectTbl[] = {
    {"0Strings", 0, sizeof(char) * 1024, CSON_STRINGS, NULL, 0, NULL, 1},
    {}
};
...
static int parseJsonArrays(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index);
static int parseJsonString(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index);
static int parseJsonStrings(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index);
...
int csonStruct2JsonObj(cson_t obj, void* input, const reflect_item_t* tbl)
{
    ...
    while (1) {
        ...
        if (ret != ERR_NONE ) {
            log_error("!!!!pack error on field:%s, cod=%d!!!!", tbl[i].field, ret);
...
int getJsonInteger(void* input, const reflect_item_t* tbl, int index, cson_t* obj)
{
    if (tbl[index].size != sizeof(char) &&
        ...
        log_error("Unsupported size(=%ld) of integer.", tbl[index].size);
        log_error("Please check if the type of field %s in char/short/int/long long!", tbl[index].field);
        return ERR_OVERFLOW;
    }
...
int getJsonString(void* input, const reflect_item_t* tbl, int index, cson_t* obj)
{
    ...
    if (tbl[index].type == CSON_STRINGS) {
...
int getJsonArray(void* input, const reflect_item_t* tbl, int index, cson_t* obj)
{
    ...
    int countIndex = -1;
    char* pSrc = NULL;
    if (tbl[index].type == CSON_ARRAY) {
        pSrc = (*(char**)((char*)input + tbl[index].offset));
    } else {
        pSrc = (char*)input + tbl[index].offset;
    }
    ...
    for (long long i = 0; i < size; i++) {
        ...
        if (ret == ERR_NONE) {
            ...
            log_error("create array item faild.");
            cson_decref(jotmp);
        }
    }
...
int getJsonReal(void* input, const reflect_item_t* tbl, int index, cson_t* obj)
{
    if (tbl[index].size != sizeof(double)) {
        log_error("Unsupported size(=%ld) of real.", tbl[index].size);
        log_error("Please check if the type of field %s is double!", tbl[index].field);
        return ERR_OVERFLOW;
    }
...
int getJsonBool(void* input, const reflect_item_t* tbl, int index, cson_t* obj)
{
    if (tbl[index].size != sizeof(char) &&
        ...
        tbl[index].size != sizeof(int) &&
        tbl[index].size != sizeof(long long)) {
        log_error("Unsupported size(=%ld) of bool.", tbl[index].size);
        log_error("Please check if the type of field %s in char/short/int/long long!", tbl[index].field);
        return ERR_OVERFLOW;
    }
...
int csonJsonObj2Struct(cson_t jo, void* output, const reflect_item_t* tbl)
{
    ...
    for (int i = 0;; i++) {
        ...
        if (jo_tmp == NULL) {
            ...
            if (jsonType == tbl[i].type || tbl[i].type == CSON_STRINGS || tbl[i].type == CSON_ARRAYS ||
        ...
        if (ret != ERR_NONE) {
            log_warn("!!!!parse error on field:%s type:%d, cod=%d!!!!", tbl[i].field, tbl[i].type, ret);
...
int parseJsonStrings(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index)
{
    const char* tempstr = cson_string_value(jo_tmp);
    if (NULL != tempstr) {
...
int parseJsonInteger(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index)
{
    ...
    if (ret != ERR_NONE) {
        log_error("Get integer failed!field:%s,errno:%d.", tbl[index].field, ret);
    } else {
...
int parseJsonArrays(cson_t jo_tmp, void* output, const reflect_item_t* tbl, int index)
{
    size_t arraySize = cson_array_size(jo_tmp);
    ...
    size_t arraySizeDef = tbl[index].size / tbl[index].arrayItemSize;
    if (arraySizeDef < arraySize) {
        log_error("input array size[%d] oversize define[%d]", arraySize, arraySizeDef);
        return ERR_OVERFLOW;
    }

    char* pMem = (char*)output + tbl[index].offset;
    memset(pMem, 0, arraySize * tbl[index].arrayItemSize);
...
long long getIntegerValueFromPointer(void* ptr, int size)
{
    ...
    if (size == sizeof(char)) {
        ...
        log_error("Unsupported size(=%d) of integer.", size);
    }
